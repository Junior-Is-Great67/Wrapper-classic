...
function listCommAssets(filters) {
	const files = DB.select("comm", filters);
	return `${header}<ugc more="0">${
		files.map(Asset.meta2StareXml).join("")}</ugc>`;
}
function listTeamAssets(filters) {
	const files = DB.select("teamassets", filters);
	return `${header}<ugc more="0">${
		files.map(Asset.meta20Xml).join("")}</ugc>`;
}
...
group
	...
	.route("POST", "/goapi/searchCommunityAssets/", async (req, res) => {
		...
		let lar = 0;
		for (const v of meta) {
			if (v.title.includes(req.body.keywords)) tXml += Asset.meta2StareXml(v);
			lar ++;
		}
		const zip = nodezip.create();
		fUtil.addToZip(zip, "desc.xml", tXml + "</theme>");
		for (const file of meta) {
			if (file.title.includes(req.body.keywords)) {
	...
	.route("POST", "/goapi/getCommunityAssets/", async (req, res) => {
		...
		console.log(listCommAssets(jason));
	...
	.route("POST", "/api_v2/asset/update/", async (req, res) => {
		...
		if (req.body.data.tags.includes("_public"))
		{
	...
	.route("POST", "/goapi/saveProp/", async (req, res) => {
		...
		info.enc_asset_id = info.file;
		res.setHeader("Content-Type", "application/xml");
		res.end("0<status=\"ok\" type=\"prop\" subtype=\"0\" title=\"" + req.body.title + "\" ptype=\"placeable\" id=\"" + info.enc_asset_id + "\" file=\"" + info.enc_asset_id + "\" enc_asset_id=\"" + info.enc_asset_id + "\" />");
	})
